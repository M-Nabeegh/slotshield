...
import { FULL_PREVIEW_URL, IS_STATIC_PREVIEW } from "../lib/publicPreview";
import type { StagingAuditResult } from "../lib/stagingContract";
import { SUPPORTED_SCENARIO_IDS } from "../lib/stagingContract";
import { makeWebsiteFindings, type WebsiteCheckResult } from "../lib/websiteAudit";
...
export function WebsiteAuditPanel({ onTryScenario }: WebsiteAuditPanelProps) {
  ...
  async function checkWebsite(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (IS_STATIC_PREVIEW) {
      setPublicResult(makeStaticPreviewResult(websiteUrl));
      ...
      <div className="website-audit-card">
        ...
        {stagingOpen ? (
          <div className="staging-audit-panel" id="staging-audit" aria-labelledby="staging-audit-title">
            ...
            {IS_STATIC_PREVIEW ? (
              <div className="staging-static-notice">
                <strong>Staging checks run on the full preview.</strong>
                <p>
                  GitHub Pages can run the fictional reliability rehearsal, but it cannot host
                  SlotShield&apos;s server-backed staging adapter.
                </p>
                <a className="button button-primary" href={FULL_PREVIEW_URL} target="_blank" rel="noreferrer">
                  Open full preview
                </a>
              </div>
            ) : (
...
function makeStaticPreviewResult(websiteUrl: string): WebsiteCheckResult {
  const requestedUrl = websiteUrl.trim() || "https://demo.slotshield.example/appointments";
  const https = requestedUrl.toLowerCase().startsWith("https://");
  const result: WebsiteCheckResult = {
    status: https ? "verified" : "attention",
    requestedUrl,
    finalUrl: requestedUrl,
    httpStatus: 200,
    responseTimeMs: 12,
    https,
    pageTitle: "Synthetic appointment page",
    contentType: "text/html",
    bookingLinks: [{ label: "Book an appointment", url: requestedUrl }],
    findings: [],
    message: https
      ? "Synthetic Pages preview only. No network request was made; open the full preview for a live public-surface check."
      : "Synthetic Pages preview only. Use an HTTPS example to see the verified state; no network request was made.",
  };

  result.findings = makeWebsiteFindings(result);
  return result;
}
